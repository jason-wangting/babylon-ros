...
export function reIndex(
    srcData: any, srcIndices: Uint32Array, srcStride: number, srcOffset: number, srcDim: number,
    destData: any, destIndices: Uint32Array, destStride: number, destOffset: number, destDim: number) {
...
export function createFloatArray(source: SourceLoader.Source, name: string, outDim: number, context: Context): Float32Array | null {
...
export function spawElements(array: any, i1: number, i2: number): void {
    var temp: any = array[i1];
    array[i1] = array[i2];
    array[i2] = temp;
}

export function insertBone(indices: any, weights: Float32Array, index: number, weight: number, offsetBegin: number, offsetEnd: number) {
...
var worldTransform: Matrix = new Matrix();
export function getWorldTransform(context: ConverterContext): Matrix {
    var mat: Matrix = worldTransform;
    mat.copyFrom(getWorldRotation(context));
    var s = getWorldScale(context);
    var matScale = Matrix.Scaling(s.x, s.y, s.z);
    mat = mat.multiply(matScale);
    return mat;
}

var worldInvTransform: Matrix = new Matrix();
export function getWorldInvTransform(context: ConverterContext): Matrix {
    var mat: Matrix = getWorldTransform(context);
    mat.copyFrom(getWorldRotation(context));
    mat.invert();
    return worldInvTransform;
}

var worldScale: Vector3 = new Vector3();
export function getWorldScale(context: ConverterContext): Vector3 {
    var scale: number = context.options.worldTransformScale.value;
    worldScale = new Vector3(scale, scale, scale);
    return worldScale;
}

var worldInvScale: Vector3 = new Vector3();
export function getWorldInvScale(context: ConverterContext): Vector3 {
    var invScale: number = 1 / context.options.worldTransformScale.value;
    worldInvScale = new Vector3(invScale, invScale, invScale);
    return worldInvScale;
}

var worldRotation: Matrix = new Matrix();
export function getWorldRotation(context: ConverterContext): Matrix {
    ...
    var mat: Matrix = worldRotation;

    switch (rotationAxis) {
        case "none": break;
        case "x": mat = Matrix.RotationX(rotationAngle); break;
        case "y": mat = Matrix.RotationY(rotationAngle); break;
        case "z": mat = Matrix.RotationZ(rotationAngle); break;
